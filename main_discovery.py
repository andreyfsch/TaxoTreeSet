...
def main():
    ...
    if args.reset and os.path.exists(args.registry):
        try:
            ...
            logger.info(f"🧹 Flag --reset ativada. Arquivo antigo removido com sucesso: {args.registry}")
        except Exception as e:
            logger.error(f"Não foi possível remover o registro antigo em {args.registry}: {e}")
            sys.exit(1)
    elif os.path.exists(args.registry):
        logger.info(f"🔄 Arquivo de registro localizado em {args.registry}. Modo de adição dinâmica/incremental ativo.")

    # 3. Validação do arquivo de mapeamento de escopos
    if not os.path.exists(args.mapping):
        logger.error(f"❌ Erro Crítico: Arquivo de mapeamento ausente em {args.mapping}")
        sys.exit(1)
    ...
    try:
        ...
        logger.info(f"🚀 Iniciando varredura taxonômica para o TaxID: {args.taxon_id}")
        ...
        logger.error(f"💥 Falha crítica durante a execução do pipeline: {e}", exc_info=True)
        sys.exit(1)
