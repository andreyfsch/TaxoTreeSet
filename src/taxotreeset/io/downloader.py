...
class NCBIDownloader:
    ...
    def download_batch(self, accessions: List[str]) -> Dict[str, List[dict]]:
        ...
        """
        logger.debug(
            f"Spawning bulk NCBI fetch call for {len(accessions)} accessions.")
        batch_results = {}
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                if not os.path.exists(zip_output_path) or os.path.getsize(zip_output_path) == 0:
                    logger.error(
                        f"Bulk download failed. Package archive is empty or missing.")
                ...
                dataset_data_root = os.path.join(
                    extract_path, "ncbi_dataset", "data")
                    ...
                    with open(fna_file, "r", encoding="utf-8") as f:
                        for line in f:
                            ...
                            if stripped.startswith(">"):
                                if current_header:
                                    local_seq_map[current_header] = "".join(
                                        current_seq_lines)

                                parts = stripped[1:].split(" ", 1)
                                current_header = parts[0]
                                seq_name = parts[1] if len(
                                    parts) > 1 else current_header
                                headers_metadata.append(
                                    {"id": current_header, "name": seq_name})
                                current_seq_lines = []
                            else:
                ...
                logger.error(
                    f"NCBI bulk dataset CLI call triggered an execution failure: {e.stderr.strip()}")
                return batch_results
            except Exception as e:
                logger.error(
                    f"Unexpected fault processing current database ingestion batch: {e}")
                return batch_results
    ...
    def download_all_pending(self, checkpoint_frequency: int = 10) -> None:
        ...
        pending_accessions = [
            acc for acc, info in accessions_dict.items() if not info.get("downloaded")]
        ...
        if total_pending == 0:
            logger.info(
                "All registered accessions are already archived inside LMDB database.")
            return
        ...
        logger.info(
            f"Grouped {total_pending} pending accessions into {len(chunks)} batch downloads (Size: {self.chunk_size}).")
